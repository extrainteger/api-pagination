    ...
    def self.included(base)
      Grape::Endpoint.class_eval do
        def paginate(collection)
          ...
          if [:pagy, "pagy"].include?(ApiPagination.config.paginator)
            Array.class_eval do
              def pagy(pagy = nil)
                @pagy = pagy
              end
              ...
              def total_pages
                @pagy.pages if @pagy.present? && @pagy.class.name.eql?("Pagy")
              end

              def limit_value
                @pagy.items if @pagy.present? && @pagy.class.name.eql?("Pagy")
              end

              def current_page
                @pagy.page if @pagy.present? && @pagy.class.name.eql?("Pagy")
              end
            end
            collection = Array.new(collection)
            collection.pagy(pagy)
          end
          return collection
        end
      end
